...
<nav x-data="{ open: false }" class="border-b border-white/70 bg-white/72 backdrop-blur-2xl">
    <div class="mx-auto flex max-w-7xl items-center justify-between gap-6 px-4 py-4 sm:px-6 lg:px-8">
        <div class="flex items-center gap-10">
            <a href="{{ auth()->check() ? route('dashboard') : route('products') }}" class="flex items-center gap-3">
            ...
            <div class="hidden items-center gap-2 lg:flex">
                @foreach ($links as $link)
                    <a href="{{ route($link['route']) }}" class="{{ request()->routeIs($link['route']) ? 'bg-[#1d1d1f] text-white shadow-[0_12px_30px_-18px_rgba(29,29,31,0.45)]' : 'text-slate-500 hover:bg-slate-100/90 hover:text-slate-900' }} rounded-full px-4 py-2 text-sm font-medium transition">
                        {{ $link['label'] }}
                    </a>
                @endforeach
            </div>
        ...
        <div class="hidden items-center gap-3 lg:flex">
            @auth
                <a href="{{ route('profile.edit') }}" class="rounded-full border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 transition hover:border-slate-300 hover:text-slate-900">
            ...
            @else
                <a href="{{ route('login') }}" class="btn btn-secondary">Masuk Admin</a>
            @endauth
        </div>
    ...
    <div x-show="open" x-transition class="border-t border-slate-200/80 bg-white/95 px-4 py-4 lg:hidden">
        <div class="space-y-2">
            @foreach ($links as $link)
                <a href="{{ route($link['route']) }}" class="{{ request()->routeIs($link['route']) ? 'bg-[#1d1d1f] text-white' : 'bg-slate-50/90 text-slate-700' }} block rounded-2xl px-4 py-3 text-sm font-medium">
                    {{ $link['label'] }}
                </a>
            @endforeach
        </div>
        ...
        <div class="mt-4 rounded-2xl border border-slate-200 bg-white/80 p-4">
            @auth
                ...
                <div class="mt-4 flex gap-2">
                    <a href="{{ route('profile.edit') }}" class="btn btn-secondary flex-1 justify-center">Profil</a>
            ...
            @else
                ...
                <div class="mt-4">
                    <a href="{{ route('login') }}" class="btn btn-secondary w-full justify-center">Masuk Admin</a>
                </div>
            @endauth
        </div>
    </div>
